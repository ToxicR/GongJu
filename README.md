...
4. 点击 **「3. 投屏」** 进入投屏窗口，有两种模式：
   - **「启动流畅投屏」**：基于 scrcpy 的低延迟视频与控制，软件会在窗口内部嵌入 scrcpy 画面。
   - **「兼容预览(截屏)」**：通过 adb 截屏渲染完整画面，帧率较低，鼠标/键盘经 adb 控制。适用于个别工控机（如部分 rk3288）硬件编码器导致 scrcpy 画面残缺时。
